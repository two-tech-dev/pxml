...
function createNvimLspHelper(cwd: string): void {
  ...
  const content = `-- pxml — XML catalog for Neovim
-- coc.nvim + coc-xml reads .vscode/settings.json automatically — no config needed.
--
-- For nvim-lspconfig + lemminx, paste into your Neovim LSP config:
--
--   local catalog = vim.fn.getcwd() .. '/.pxml/catalog.xml'
--   local lspconfig = require('lspconfig')
--   lspconfig.lemminx.setup({
--     settings = { xml = { catalogs = { catalog } } }
--   })
`;
