...
program
    ...
    for (const nodeId of order) {
        ...
        try {
            ...
            const cachedTestHash = cached?.test_xml_hash;
...
function injectHistoricalBugs(nodes, cwd) {
    const bugsHistoryPath = path.join(cwd, 'bugs_history.xml');
    if (!fs.existsSync(bugsHistoryPath))
        return;
    try {
        ...
        for (const node of nodes) {
            for (const bug of rawBugs) {
                ...
                const hasExplicitLearned = node.constraints.some((c) => c.learnedFrom === bugId);
                if (matchesFlow || hasExplicitLearned) {
                    const alreadyExists = node.constraints.some((c) => c.learnedFrom === bugId || c.description.includes(bugDesc));
                    if (!alreadyExists) {
...
function calculateEstimatedCost(model, stats) {
